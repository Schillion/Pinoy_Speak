...
export default function Translator() {
  ...
  useEffect(() => {
    ...
    // Only true grammatical function words — never content words that could
    // later acquire slang meanings (e.g. "grabe" graduated to slang status).
    const STOP = new Set([
      // Filipino grammatical particles
      "ang","ng","sa","na","at","ay","si","ni","para","pero","kasi","lang",
      "din","rin","po","daw","raw","pala","naman","talaga","sana","yung","ung",
      // Filipino conjunctions / adverbs that will never be slang
            ...
            <motion.section variants={fadeUp}>
              ...
              {(() => {
                const classes = new Set(
                  result.tokens.map((t) => result.results[t.replace(/[.,!?'"]/g, "")]?.classification ?? "unknown")
                );
                return (
                  <div className="flex flex-wrap gap-2 mb-3 -mt-1">
                    {classes.has("slang")   && <span className="badge-slang text-[11px] pointer-events-none">slang</span>}
                    {(classes.has("standard") || classes.has("unknown")) && <span className="badge-regular text-[11px] pointer-events-none">regular</span>}
                    {classes.has("profane") && <span className="badge-censored text-[11px] pointer-events-none">profane</span>}
                  </div>
                );
              })()}
              {verifying.size > 0 && (
